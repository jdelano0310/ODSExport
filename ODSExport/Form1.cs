				...
				if (sfd.ShowDialog() == DialogResult.OK)
				{
					...
					if (!fileError)
					{
						try
						{
							...
							string columnNames = "";
							string[] outputCsv = new string[dgvSheet.Rows.Count + 1];
							...
							if (chkFirstRowColumnNames.Checked)
							{
								...
								{
									columnNames += dgvSheet.Columns[i].HeaderText.ToString() + ",";
								}
							...
							for (int i = 1; (i - 1) < dgvSheet.Rows.Count; i++)
							{
								for (int j = 0; j < columnCount; j++)
								{
									if (dgvSheet.Rows[i - 1].Cells[j].Value.ToString().Contains(','))
									{
										...
										outputCsv[i] += $"\"{dgvSheet.Rows[i - 1].Cells[j].Value.ToString()}\",";
									}
									else
									{
										outputCsv[i] += dgvSheet.Rows[i - 1].Cells[j].Value.ToString() + ",";
